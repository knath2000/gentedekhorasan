...
const SettingsToggle = ({ label, settingKey, class: className }: ToggleSwitchProps) => {
  let store;
  if (settingKey === 'autoplay') {
    store = autoplayEnabled;
  } else if (settingKey === 'translation') {
    store = showTranslation;
  } else {
    console.error(`Unknown settingKey: ${settingKey}`);
    return null; 
  }
